...
- **MAE**: Mean Absolute Error and calculated by averaging the distances between the absolute target values and the predicted values. So, it doesn't penalize heavily huge errors as in case of MSE. So, it is not as sensitive as MSE or influenced by any outliers.
